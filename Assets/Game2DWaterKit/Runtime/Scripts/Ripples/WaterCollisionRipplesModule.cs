        ...
        private void CreateWaterEnterExitRipples(Collider2D objectCollider, bool isObjectEnteringWater)
        {
            ...
            for (int surfaceVertexIndex = startIndex; surfaceVertexIndex <= endIndex; surfaceVertexIndex++)
            {
                ...
                var contactFilter = new ContactFilter2D
                {
                    ...
                    useDepth = true,
                    minDepth = _collisionMinimumDepth,
                    maxDepth = _collisionMaximumDepth,
                    useTriggers = !_collisionIgnoreTriggers
                };
                var hitCount = Physics2D.Raycast(surfaceVertexPosition, raycastDirection, contactFilter, _raycastResults, _collisionRaycastMaximumDistance);
#else
