namespace Game2DWaterKit
{
    ...
    public class LargeWaterAreaManager : MonoBehaviour
    {
        ...
        private List<SharedRenderingOutput> _sharedRenderingOutputs = new List<SharedRenderingOutput>();
        internal class SharedRenderingOutput
        {
        ...
        private void Start()
        {
            ...
            if (waterObjectBoxCollider2D != null)
            {
                ...
                waterObjectBoxCollider2D.compositeOperation = Collider2D.CompositeOperation.Merge;
#else
...
#region Methods
        ...
        {
            return GetSharedRenderingOutput(currentRenderingCamera).lastRenderedFrame == _frameCount;
        }

        internal void MarkCurrentFrameAsRendered(Camera currentRenderingCamera)
        {
            GetSharedRenderingOutput(currentRenderingCamera).lastRenderedFrame = _frameCount;
        }

        internal SharedRenderingOutput GetSharedRenderingOutput(Camera renderingCamera)
        {
            for (int i = 0, imax = _sharedRenderingOutputs.Count; i < imax; i++)
            {
                if (_sharedRenderingOutputs[i].renderingCamera == renderingCamera)
                    return _sharedRenderingOutputs[i];
            }
            var sro = new SharedRenderingOutput();
            sro.renderingCamera = renderingCamera;
            _sharedRenderingOutputs.Add(sro);
            return sro;
        }
